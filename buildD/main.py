...
if banAthleteBuild: #if banAthleteBuild variable is true, build those tables
    dopefile = 'doping.csv'
    aid = 1
    bid = 1
...
with open(eventfile, 'r') as csvfile:
    spamreader = csv.reader(csvfile, delimiter=',')
    for row in spamreader:
        date = ""
        name = ""
...
# if eventBuild:
#     print("Building the event table using webscraped data")
...
#     # print(results.text)
#     rJSON = json.loads(results.text)
#     print(rJSON)



conn.commit() #send all q# ueries into SQL
print("\n-------------------\nFINISHED\n-------------------\n")
